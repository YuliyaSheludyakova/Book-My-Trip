...
public interface MuseumRepository extends EntryRepository<Museum> {
	...
	default List<Museum> filterByPriceLevel(List<Museum> maybeMuseums,
	...
	private List<Museum> findByCityAndPriceLevelOrderByPriceLevel(String city, int priceLevel) {
	...
	default List<Museum> filterByMuseumType(List<Museum> maybeMuseums,
			                                String museumType, String city) {
		if (maybeMuseums != null && museumType != null) {
			maybeMuseums
			    .retainAll(findByCityAndMuseumTypeOrderByName(city, museumType));
