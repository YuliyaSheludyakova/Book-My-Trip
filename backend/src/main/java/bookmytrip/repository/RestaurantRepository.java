...
public interface RestaurantRepository extends EntryRepository<Restaurant> {
	...
	default List<Restaurant> filterByPriceLevel(List<Restaurant> maybeRestaurants,
			                                    Integer priceLevel, String city) {
		if (maybeRestaurants != null && priceLevel != null) {
			maybeRestaurants
			    .retainAll(findByCityAndPriceLevelOrderByPriceLevel(city,priceLevel));
		} else if (priceLevel != null) {
			maybeRestaurants = findByCityAndPriceLevelOrderByPriceLevel(city, priceLevel);
		}
		return maybeRestaurants;
	}

	private List<Restaurant> findByCityAndPriceLevelOrderByPriceLevel(String city,
			                                                          int priceLevel) {
	...
	private int compareByPriceLevel(Restaurant firstRestaurant, Restaurant secondRestaurant) {
		return firstRestaurant.getPriceLevel()
				   .compareTo(secondRestaurant.getPriceLevel());
	}

	default List<Restaurant> filterByCuisine(List<Restaurant> maybeRestaurants,
			                                 String cuisine, String city) {
		if (maybeRestaurants != null && cuisine != null) {
			maybeRestaurants
			    .retainAll(findByCityAndCuisineOrderByName(city, cuisine));
	...
	private List<Restaurant> findByCityAndCuisineOrderByName(String city, String cuisine) {
		return findByCity(city).stream()
				   .filter(r -> isMatchByCuisine(r, cuisine))
				   .sorted((r1, r2) -> compareByName(r1, r2))
				   .collect(Collectors.toList());
	}
